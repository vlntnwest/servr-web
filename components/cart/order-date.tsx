...
import { getSchedulableSlots, PREP_MINUTES } from "@/lib/opening-hours";
...
export default function OrderDate() {
  ...
  const asapAvailable = restaurantCtx?.restaurant.isOpen ?? false;

  const availableDays = useMemo(() => {
    const r = restaurantCtx?.restaurant;
    if (!r) return [];
    return getSchedulableSlots(
      openingHours,
      restaurantCtx?.exceptionalHours ?? [],
      r.timezone,
      r.preparationLevel,
    );
  }, [openingHours, restaurantCtx?.exceptionalHours, restaurantCtx?.restaurant]);
  ...
  const estimatedReadyTime = useMemo(() => {
    ...
    if (!lvl || !asapAvailable) return null;
    const minutes = PREP_MINUTES[lvl];
